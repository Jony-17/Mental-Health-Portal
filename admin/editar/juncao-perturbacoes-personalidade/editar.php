                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold text-primary">Junção de Perturbações de Personalidade
                            </h6>
                        </div>
                        ...
                        <div class="card-body">
                            ...
                            if (isset($_POST['edit_btn'])) {
                                $juncao_pert_pers_id = $_POST['edit_id'];

                                $query = "SELECT
                                                j.juncao_pert_pers_id,
                                                j.perturbacoes_id,
                                                pp.nome AS pert_pers,
                                                p.nome AS nome_perturbacao,
                                                j.grupos_perturbacoes_id,
                                                g.nome AS nome_grupo,
                                                p.perturbacoes_personalidade_id
                                            FROM
                                                juncao_pert_personalidade j
                                            INNER JOIN
                                                perturbacoes pp ON j.perturbacoes_id = pp.perturbacoes_id
                                            INNER JOIN
                                                perturbacoes_personalidade p ON j.perturbacoes_personalidade_id = p.perturbacoes_personalidade_id
                                            INNER JOIN
                                                grupos_perturbacoes g ON j.grupos_perturbacoes_id = g.grupos_perturbacoes_id
                                            WHERE
                                                j.juncao_pert_pers_id = '$juncao_pert_pers_id'";
                                ...
                                if (mysqli_num_rows($query_run) > 0) {
                                    ...
                                    <form action="juncao-perturbacoes-personalidade.php" method="post"
                                        ...
                                        <input type="hidden" name="juncao_pert_pers_id"
                                            value="<?php echo $row['juncao_pert_pers_id'] ?>">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="nome_perturbacao" class="form-control">
                                                ...
                                                while ($row_perturbacao = mysqli_fetch_assoc($result_perturbacoes)): ?>
                                                    <option value="<?php echo $row_perturbacao['perturbacoes_id']; ?>" <?php if ($row_perturbacao['perturbacoes_id'] == $row['perturbacoes_id'])
                                                           echo 'selected'; ?>>
                                        ...
                                        <div class="form-group">
                                            <label>Nome do Grupo da perturbação</label>
                                            <select name="nome_grupo" class="form-control">
                                                <?php
                                                ...
                                                while ($row_grupo = mysqli_fetch_assoc($result_grupos_perturbacoes)): ?>
                                                    <option value="<?php echo $row_grupo['grupos_perturbacoes_id']; ?>" <?php if ($row_grupo['grupos_perturbacoes_id'] == $row['grupos_perturbacoes_id'])
                                                           echo 'selected'; ?>>
                                        ...
                                        <div class="form-group">
                                            <label>Nome do Grupo associados ao grupo da perturbação</label>
                                            <select name="nome_grupo_assoc_perturbacao" class="form-control">
                                                <?php
                                                $query_grupos_assoc_perturbacoes = "SELECT perturbacoes_personalidade_id, nome FROM perturbacoes_personalidade";
                                                $result_grupos_assoc_perturbacoes = mysqli_query($conn, $query_grupos_assoc_perturbacoes);

                                                while ($row_grupo_assoc_perturbacao = mysqli_fetch_assoc($result_grupos_assoc_perturbacoes)): ?>
                                                    <option value="<?php echo $row_grupo_assoc_perturbacao['perturbacoes_personalidade_id']; ?>"
                                                        <?php if ($row_grupo_assoc_perturbacao['perturbacoes_personalidade_id'] == $row['perturbacoes_personalidade_id'])
                                                            echo 'selected'; ?>>
                                                        <?php echo $row_grupo_assoc_perturbacao['nome']; ?>
                                                    </option>
                                                <?php endwhile; ?>
                                            </select>
                                        </div>
